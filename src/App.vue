...
<div class="frame" :class="isday ? 'day' : 'night'">
   <form class="search-location" @submit.prevent="fetchData">
        <input
          type="text"
          class="form-control text-muted form-rounded shadow-sm"
...
<div class="box">
  ...
  <div class="front">
      ...
      <div class="preview" style="margin-top:32px">{{ dateBuilder() }}</div>
		</div>
...
export default {
  ...
  data () {
    return {
      ...
      key:'56b465a77cbf30d7045c756d7c8838ea',
    }
  },
  methods:{
    fetchData: function(){
         fetch(`https://api.openweathermap.org/data/2.5/weather?q=${this.weatherCity}&units=metric&APPID=${this.key}`)
          ...
          .then(data=>{
              ...
              const daylight = data.weather[0].icon
              if (daylight.includes("n")) {
                this.isday = false
              }
              if(daylight.includes("d")){
                this.isday =true
              }
              ...
              if(data.weather[0].main.includes("Clear")){
               ...
               if(data.weather[0].main.includes("Clear night")){
                 this.cloudy = true
                this.clearSky = false
                this.strom=true
     ...
     dateBuilder () {
      ...
      let days = this.weather.cityName;
